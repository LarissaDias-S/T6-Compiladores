...
class CodeGenerator:
    ...
    def _gerar_imports(self, ast: ProgramaNode) -> List[str]:
        linhas = ["import sys", "import os", "import joblib", "import pandas as pd", "import numpy as np"]
        linhas.append(
    ...
    def _gerar_treinamento(self, modelo: ModeloNode, metricas: List[str]) -> List[str]:
        ...
        # 2. Busca no dicionário de forma segura. Se achar, pega a classe do scikit-learn.
        if alg_limpo in _ALGORITMO_IMPORTS:
    ...
    def _gerar_algoritmo(self, modelo: ModeloNode) -> str:
        ...
        # 1. Limpa aspas e espaços que possam ter vindo da árvore sintática (AST)
        alg_limpo = str(modelo.algoritmo).replace('"', '').replace("'", "").strip()
