...
class CodeGenerator:
    ...
    def _gerar_imports(self, ast: ProgramaNode) -> List[str]:
        linhas = ["import os", "import joblib", "import pandas as pd", "import numpy as np"]
        linhas.append("from sklearn.model_selection import train_test_split")
    ...
    def _gerar_treinamento(self, modelo: ModeloNode, metricas: List[str]) -> List[str]:
        classe = _ALGORITMO_IMPORTS[modelo.algoritmo][1]
        ...
        linhas = [
            ...
            "",
            f"print('=== Métricas — {modelo.nome} ({modelo.algoritmo}) ===')",
        ]

        for metrica in metricas:
            linhas.extend(self._gerar_calculo_metrica(metrica, modelo.nome, modelo.algoritmo))
